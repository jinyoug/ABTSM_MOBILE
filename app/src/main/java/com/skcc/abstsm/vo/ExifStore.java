...
public class ExifStore {
    ...
    public void MarkGeoTagImage(String imagePath, Location location)
    {
        try {
            ...
            //Log.e("log1" , "exif_la0t위치 : " +  GpsInfo.convert(location.getLatitude()));
            //Log.e("log1" , "exif_long위치 : " +  GpsInfo.convert(location.getLongitude()));


            SimpleDateFormat fmt_Exif = new SimpleDateFormat("yyyy:MM:dd HH:mm:ss");
            exif.setAttribute(ExifInterface.TAG_DATETIME,fmt_Exif.format(new Date(location.getTime())));
            Log.e("log1" , "저장 시간 : " +  fmt_Exif.format(new Date(location.getTime())));
            exif.saveAttributes();
        } catch (IOException e) {
            e.printStackTrace();
        }
